...
public interface BrowseRecordRespository extends CrudRepository<BrowseRecord,Integer>,PagingAndSortingRepository<BrowseRecord,Integer>{
    ...
    @Query(value = "select br from BrowseRecord br where br.customer=?1 and br.time=(select max(xbr.time) from BrowseRecord xbr where xbr.goods=br.goods) group by br.goods.goodsId")
