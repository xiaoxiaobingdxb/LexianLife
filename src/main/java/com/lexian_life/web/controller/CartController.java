...
public class CartController {
    ...
    public void addToCart(int goods_id, @RequestParam(required = false) Integer num, HttpSession session,HttpServletResponse response) {
        if (num == null) num = 1;
        Cart cart = (Cart) session.getAttribute("cart");
        if (cart == null) {
            Customer customer = (Customer) session.getAttribute("customer");
     ...
     * @param response
     */
    @RequestMapping(value = "/updateCartItemNum",method = {RequestMethod.GET})
    public void updateCartItemNum(int cartItemId,int num,HttpSession session,HttpServletResponse response){
        Cart cart = (Cart) session.getAttribute("cart");
     ...
     * @param response
     */
    @RequestMapping(value = "/selectCartItem",method = {RequestMethod.GET})
    public void selectCartItem(int cartItemId,int selected,HttpSession session,HttpServletResponse response){
        JSONObject jsonObject = new JSONObject();
        Cart cart = (Cart) session.getAttribute("cart");
        for(int i=0;i<cart.getCartItems().size();i++){
            if(cart.getCartItems().get(i).getCartItemId()==cartItemId){
    ...
    @RequestMapping(value = "/toSettleAccount",method = {RequestMethod.GET,RequestMethod.POST})
    ...
    @RequestMapping(value = "/addAddressWidthAccount",method = {RequestMethod.POST})
    public void addAddressWithAccount(RecAddr getRecAddr, HttpServletRequest request,HttpServletResponse response){
            try {
     ...
     * @param response
     */
    @RequestMapping(value = "/selectAddressWidthAccount",method = {RequestMethod.GET})
    public void selectAddressWidthAccount(int recaddrId,HttpSession session,HttpServletResponse response){
