...
public class CommonController {
    ...
    public String requestMain(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
        enter(request,response);
        if(request.getSession().getAttribute("catalogs")==null) {
            List<Catalog> catalogs = goodsService.displayCatalogs();
            for(int i=0;i<catalogs.size();i++){//默认给每个Category查询20个Goods
