...
import com.lexian_life.service.OrderService;
import com.lexian_life.util.ImageUtil;
...
public class CustomerController {
    ...
    public void register(Customer customer,HttpServletRequest request,HttpServletResponse response){
        try {
            try {
                ...
                switch (e.getErrorCode()) {
                    case 0:
                        ...
                        request.getRequestDispatcher("/page/foreground/user/Register.jsp").forward(request, response);
                        return;
                    case 1:
    ...
    public void login(HttpSession session,HttpServletRequest request,HttpServletResponse response, String username, String password,String verification,String rempsw, String autoLogin) throws ServletException, IOException {
     ...
     * @param response
     */
    @RequestMapping(value = "/customerModify",method = {RequestMethod.GET,RequestMethod.POST})
    public void customerModify(@RequestParam("headFile") MultipartFile headFile, Customer getCustomer, HttpServletRequest request, HttpServletResponse response,HttpSession session) {
        try{
            try {
            ...
            if(!"".equals(headFile.getOriginalFilename())) {//选择了新头像
                File file = new File(path, customer.getUsername() + headFile.getOriginalFilename().substring(headFile.getOriginalFilename().lastIndexOf(".")));
...
//            return;
     ...
     * @param response
     */
    @RequestMapping(value = "/sendModifyPswEmail",method = {RequestMethod.POST,RequestMethod.GET})
    ...
    public void modifyPassword(String password,String getCode,HttpServletRequest request,HttpServletResponse response){
        try {
            ...
            if (cookie != null) {
                ...
                if (code.equals(getCode)) {//验证码正确
                    ...
                    try {
                        customerService.updateCustomer(customer);
                        if(ServletUitl.getCookieByName(request.getCookies(),"autoLogin")!=null||ServletUitl.getCookieByName(request.getCookies(),"remPsw")!=null){
     ...
     * @param response
     */
    @RequestMapping(value = "/sendModifyEmailEmail",method = {RequestMethod.POST,RequestMethod.GET})
    ...
    public void modifyEmail(String email,String getCode,HttpServletRequest request,HttpServletResponse response){
        try {
     ...
     * @return
     */
    @RequestMapping(value = "/toEditAddr",method = {RequestMethod.GET})
    public void toEditAddr(int id,HttpServletRequest request,HttpServletResponse response){
        try {
            ...
            request.getRequestDispatcher("/page/foreground/user/EditAddress.jsp").forward(request,response);
            return;
        }catch (IOException|ServletException e){
            e.printStackTrace();
        }
    ...
    public void updateAddr(RecAddr getRecAddr,HttpServletRequest request,HttpServletResponse response){
        try {
     ...
     * @return
     */
    @RequestMapping(value = "/delRecAddr",method = {RequestMethod.GET,RequestMethod.POST})
    ...
    public void queryOrderCountByStatus(HttpSession session,HttpServletResponse response){
        ...
        try {
            ...
            jsonObject.put("waitpaynum",orderService.getOrderCountByStatus(customer, OrderStatus.NOTPAY));
            jsonObject.put("waitsendnum",orderService.getOrderCountByStatus(customer,OrderStatus.PAYED)+orderService.getOrderCountByStatus(customer,OrderStatus.READY));
            jsonObject.put("waitrecnum",orderService.getOrderCountByStatus(customer,OrderStatus.NOTRECEIVE));
     ...
     * @param response
     */
    @RequestMapping(value = "/getAttentions",method = {RequestMethod.GET})
     ...
     * @param response
     */
    @RequestMapping(value = "/getBrowsedGoods",method = {RequestMethod.GET})
    public void getBrowsedGoods(int pageIndex,HttpServletRequest request,HttpServletResponse response){
        String encoding = "utf-8";
        response.setContentType("text/plain;charset=" + encoding);
        response.setCharacterEncoding(encoding);
        PrintWriter printWriter = null;
    ...
    public void getCollection(int pageIndex,HttpSession session,HttpServletResponse response){
        ...
        try {
            for(int i=0;i<attentions.size();i++){
                ...
                jsonObject.put("monthSaleCount",goodsService.getCurMonthSaleCount(attention.getGoods().getGoodsId()));
                array.put(jsonObject);
            }
            printWriter = response.getWriter();
            printWriter.println(array.toString());
     ...
     * @param response
     */
    @RequestMapping(value = "/commentGoods",method = {RequestMethod.GET})
    public void commentGoods(int orderItemId,String content,int score,HttpSession session,HttpServletResponse response){
        String encoding = "utf-8";
        response.setContentType("text/plain;charset=" + encoding);
        response.setCharacterEncoding(encoding);
        PrintWriter printWriter = null;
