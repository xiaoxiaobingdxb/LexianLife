                ...
                <div class="orderDetail">
                    <table class="table">
                        <tr>
                            ...
                            <td style="border: none"><cfm:formatDate value="${sessionScope.order.createTime}" pattern="yyyy-MM-dd hh:mm:ss"/></td>
                        </tr>
                        <tr>
                    ...
                    <form action="#" method="post">
                        <table class="table">
                            <c:forEach items="${sessionScope.payways}" varStatus="status" step="4">
                                <tr>
                                    <td onclick="selectPayway(this)" id="${sessionScope.payways[status.index].paywayId}"><img src="${sessionScope.payways[status.index].img}">${sessionScope.payways[status.index].name}</td>
                                    <c:if test="${sessionScope.payways[status.index+1]!=null}">
                                        <td onclick="selectPayway(this)" id="${sessionScope.payways[status.index+1].paywayId}"><img src="${sessionScope.payways[status.index+1].img}">${sessionScope.payways[status.index+1].name}</td>
                                    </c:if>
                                    <c:if test="${sessionScope.payways[status.index+2]!=null}">
                                        <td onclick="selectPayway(this)" id="${sessionScope.payways[status.index+2].paywayId}"><img src="${sessionScope.payways[status.index+2].img}">${sessionScope.payways[status.index+2].name}</td>
                                    </c:if>
                                    <c:if test="${sessionScope.payways[status.index+3]!=null}">
                                        <td onclick="selectPayway(this)" id="${sessionScope.payways[status.index+3].paywayId}"><img src="${sessionScope.payways[status.index+3].img}">${sessionScope.payways[status.index+3].name}</td>
                                    </c:if>
                                </tr>
                            </c:forEach>
                        </table>
                        <a href="/pay.do"><button  class="submtPayBtn pull-right" type="button">前往支付</button></a>
                    </form>
                </div>
            </div>
        </div>
