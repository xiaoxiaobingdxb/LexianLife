            ...
            <div class="myCartTable">
                <table class="table">
                    ...
                    <c:forEach var="cartItem" items="${sessionScope.cart.cartItems}">
                        <tr>
                            <td><input type="checkbox" id="check${cartItem.cartItemId}" <c:if test="${cartItem.selected==1}">checked</c:if> onclick="selectCartItem(${cartItem.cartItemId})"> </td>
                        <td>
                            <div class="row">
                                <div class="col-md-4">
                        ...
                        <td>
                            <div class="numBox">
                                ...
                                <input type="number" value="${cartItem.num}" oninput="numChange(this,'${cartItem.cartItemId}')" id="${cartItem.cartItemId}">
                                <button class="subBtn" onclick="sub('${cartItem.cartItemId}')">-</button>
                                <br><c:if test="${cartItem.num<=cartItem.goods.inventory}">有货</c:if><c:if test="${cartItem.num>cartItem.goods.inventory}">无货</c:if>
                            </div>
                        </td>
                            <td>￥<span id="account${cartItem.cartItemId}">${cartItem.goods.unitPrice*cartItem.num}</span></td>
                        <td>
                            <a class="btn btn-danger btn-sm" href="/removeCartItem.do?cartItemId=${cartItem.cartItemId}">删除</a>
                        </td>
                    </tr>
                    </c:forEach>
                </table>
                <div class="operateBox pull-right">
                    已选择<span class="selectNum" id="selectNum">${sessionScope.cart.getSelectSize()}</span>件商品&nbsp;总价:<span class="allPrice" id="allPrice">￥${sessionScope.cart.getTotalPrice()}</span>
                    <a href="/toSettleAccount.do"><button class="gotoPayBtn">去结算</button></a>
                </div>
            </div>
        </div>
